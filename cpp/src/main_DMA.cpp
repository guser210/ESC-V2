...
void setDutyCycle(uint16_t dc) {
	...
	if (dc < 1) {
		//GPIOA->BRR = OD_B_Pin |	OD_A_Pin | OD_C_Pin; // C
		stopMotors();

	} else if (dmaBuffer[0] == 0 && dc >= 0) {
		startMotors();
	}
	if (dc > resolution)
		dc = resolution;
...
void stopMotors() {
	...
	TIM3->DIER &= ~(diOff[0] | diOff[1] | diOff[2]);
	TIM3->CCER &= ~(ccOff[0] | ccOff[0] | ccOff[0]);
	EXTI->IMR1 = resetImrFlags;
	EXTI->FTSR1 = 0;
	EXTI->RTSR1 = 0;
	HAL_Delay(50);
	GPIOA->BRR = OD_B_Pin | OD_A_Pin | OD_C_Pin;
	; //odOff[0];
...
void startMotors() {
	GPIOA->BRR = OD_B_Pin | OD_A_Pin | OD_C_Pin; // odOff[0];
//	GPIOA->BRR = odOff[1];
//	GPIOA->BRR = odOff[2];
	EXTI->FTSR1 = 0;
	EXTI->RTSR1 = 0;
...
//	TIM3->CCER &= ~(ccOff[powerStep]);
	TIM3->CCER &= ~(ccOff[0] | ccOff[1] | ccOff[2]);
...
//	if( motorSpeedCurrent == 0)
//		motorSpinning = 0;
	if ( dmaBuffer[0] == 0 && TIM7->ARR == 0) //|| motorSpeedCurrent == 0)
	{
		HAL_NVIC_DisableIRQ(EXTI4_15_IRQn);
	}
...
inline void processDmaSignalDShot() {
	...
	//static uint8_t directionCounter = 0;
...
//			directionCounter = 0;
	...
	errorReads = 0;
	newDmaSignal = 0;
	motorSpeed = throttle > 1 ? throttle + 1 : 0; // throttle;
}
...
const double batteryCellLow = 3.3;
const double batteryCellHigh = 4.21;
...
//		TIM3->ARR = map(RPM,0,2000,3072-1,512-1);
		...
		if (commutationCNT > 1 && motorSpeedCurrent > 0) {
			//TIM7->CNT = 0;
			// each map call takes 106 clock cycles.
			if (newDmaSignal == 1)
...
//				RPM = eRPM / 7;
		...
		if (motorSpeedCurrent == 0 && (REV_GPIO_Port->IDR & REV_Pin) == 0) { // switch direction when button pressed.
...
//					processDmaSignalDShot();
		...
		if (motorSpeed != motorSpeedCurrent || tempF > maxTempF) {
			if (motorSpeed > 250 && tempF > maxTempF) {
...
//					}
				if (motorSpeedCurrent < 2000) {
					if (motorSpeed > motorSpeedCurrent) {
						if ((motorSpeed - motorSpeedCurrent) > 5)
							motorSpeedCurrent += 5;
						else
							motorSpeedCurrent++;

					} else {
						if ((motorSpeedCurrent - motorSpeed) > 5)
							motorSpeedCurrent -= 5;
						else
							motorSpeedCurrent--;
